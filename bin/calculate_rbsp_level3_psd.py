...
from dasilva_invariants.insitu import InSituObservation, get_rbsp_electron_level3
...
def _do_task(
    ...
    try:
        result = psd.calculate_LStar_profile(
            fixed_mu=mu,
            fixed_K=K,
            insitu_observation=insitu_observation,
            mesh=mesh,
            particle=particle,
            calculate_lstar_kwargs={"num_local_times": 20},
        )
...
def _get_mesh_files(data_dir: str, run_name: str) -> Tuple[NDArray[Any], NDArray[Any]]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--particle", default="electron", choices=["electron", "proton"]
    )
    ...
    if args.particle == "electron":
        ...
        raise NotImplementedError("Only electron data implemented at this time")
