...
def process_hdf_path(hdf_path, radius, pitch_angle):
    ...
    output = {"filename": os.path.basename(hdf_path)}
    ...
    try:
        result = invariants.calculate_K(mesh, starting_point, pitch_angle=pitch_angle)
    ...
    try:
        result = invariants.calculate_LStar(
            mesh, starting_point, starting_pitch_angle=pitch_angle, num_local_times=15
        )
    except invariants.FieldLineTraceInsufficient:
        result = None
        output[key] = -1.0
    ...
    for hdf_path in hdf_paths[::10]:
        for radius in radii:
            for pitch_angle in pitch_angles:
                tasks.append(
                    dask.delayed(process_hdf_path)(hdf_path, radius, pitch_angle)
                )
